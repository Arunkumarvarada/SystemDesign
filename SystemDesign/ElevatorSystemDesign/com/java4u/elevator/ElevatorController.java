...
public class ElevatorController implements IElevatorController, ElevatorEventListener {
	...
	private void initFloors(int numFloors) {
		...
		for (int i = 0; i < numFloors; i++) {
			passengers.add(i, new Queue<Integer>());
		}
	...
	public void status() {
		...
		for (Elevator el : elevators) {
			System.out.println(" elID =  " + i++ + " CurrentFloor = " + el.getCurrentFloor() + " Moving =  "
					+ el.getMove() + " DIRECTION = " + el.getDirection() + "\n");
		}
	}
	...
	public void pickup(int floor, boolean direction) {
		...
		// on the same floors and they are picked up at the same time from different
		// floors
		...
		for (int elID : elevatorIDs) {
			...
			if ((elevator.getMove() == false)
					...
					|| ((userDirection == Elevator.DIRECTION.DOWN)
							&& (elevator.getDirection() == Elevator.DIRECTION.DOWN)
							&& (floor <= elevator.getCurrentFloor())))
